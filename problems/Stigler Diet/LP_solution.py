...
def main():
    ...
    ################ Decission Variables.
    foods = [solver.NumVar(lb=0.0, ub=solver.infinity(), name=item[0]) for item in data]
    ...
    for i, nutrient in enumerate(nutrients):
        print('{}: {:.2f} (min {})'.format(nutrient[0], nutrients_result[i],
                                           nutrient[1]))
    print('')
    print('Problem solved in ', solver.wall_time(), ' milliseconds, ', solver.iterations(), ' iterations')
